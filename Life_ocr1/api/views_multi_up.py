...
#         # output_data["Annualised_Net_Premium"] = output_data["Total_Premium_With_Tax|Collected_premium"]*12-output_data["GST_AMOUNT"]*12
#         # gst_components = []
...
# # ✅ Final Django View
# class Gemini_Bulk_OCRView(APIView):
...
#         if not filenames and not uploaded_file:
#             return Response({"error": "No filename or file provided"}, status=400)

#         results = []
#         for fname in filenames:
#                 file_url = base_url + quote(fname)
...
#                     results.append({"filename":fname, "error":"Download failed"})

#                     with open(temp_path, "wb") as file:
#                         file.write(response.content)

#                     result = pdf_processing(temp_path)
#                     if isinstance(result.data, dict):
#                         results.append({"filename":fname, "data": result.data})
#                     else:
#                         result.append({"filename":fname,"error": "processing failed"})
#             elif uploaded_file:
